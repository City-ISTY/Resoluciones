...
4.2.1| Proyecto Académico 1

4.2.2| Proyecto Académico 2
...
Al final de los puntos, se coloca la sección "**CAMBIOS**", la cual es opcional.
...
2.  **Aprobación final:** Para aprobar definitivamente, responda con: "APROBADO" con mayúscula o minúscula.
...
3. Se notifica mediante correo a secretaría la creación de la convocatoria, enviando adicionalmente el enlace a un documento excel, para toma de datos. (Ver **Toma de Registros**)

4. Genera un documento formal en formato PDF,  (**CONVOCATORIA**) el cual lo envía a cada uno de los asistentes secuencialmente para que se firmen, una vez firmado por todos, se envía el documento listo a secretaría. (Ver **Firmado de Documentos**)
...
Se adjunta el links para la toma de asistencia y votos:
...
En el correo enviado a secretaría bajo el asunto *PROGRAMACIÓN SESIÓN EXTRAORDINARIA* se proporcionó un enlace a un archivo excel, los cuales se utilizan en esta sección.
...
**IMPORTANTE**: En este paso es de suma importancia asegurarse de que al grabar la reunión se configure el idioma "Español-Mexico".

Toda la transcripción generada de la reunión, es obtenida por el robot para la generacion de los documentos, por lo que si se realiza mal este paso, no se podrá obtener la información tratada en la reunión.

## **Paso 2:** Registro de Asistencia

Para realizar el registro de asistencia, hay que ubicarse en la hoja *ASISTENCIA* del archivo excel, para luego marcar con una "x" en la columna **asistencia**. Para los asistentes que no asistieron, se deja el campo vacío.
  
| num | id_asistente| nombre| rol | asistencia |
|-----|--------------|------|------|------------|
| 1 | mfxkguhmjxwrm17ztri | Mgtr Javier Hernando Espinoza Garzón | Rector |  |
| 2 | mfxkguhmbrgs57ekugd | Dania Portelles Cobas | Vicerrectora Académica |  |
| 3 | mfxkguhmp7i4eprk38q | Carlos Eduardo Estrella Carrera | Vicerrector Académico |  |

  ### EXPLICACION DE LAS COLUMNAS 
  - num: numero de la fila, lo utiliza el robot para colocar la información
  - is_asistente: identificador del asistente, lo utiliza el robot internamete.
  - nombre: nombre del asistente
  - rol: función o cargo
  - asistencia: Registro que se marca con "X" mayuscula o minúscula, para indicar la presencia de un asistente (Se deja vacío en caso de ausencia).
...
- num: numero de la fila, lo utiliza el robot para colocar la información
- nombre: nombre del asistente
- id_tema: identificador del tema, lo utiliza el robot para realizar calculos
- tema: Tema tratado en la reunión
- opcion: Es la votación que realiza el asistente, únicamente se pueden insertar los siguientes valores. (Se utiliza un menu de selección en donde se tendrá que elegir el valor)
		...
		- Abstencion
- notificacion: Es un texto explicando al área que se tiene que notificar, (*Posteriormente este texto se coloca textualmente en el "Artículo 2: Notificar a ..." del documento de resolucion generado*.)

- reunion_terminada : Permite activar la segunda parte del robot, para ello colocar "x" en cualquier sitio de la columna. Se llena únicamente cuando se haya acabado la reunión, se hayan rellenado todos los campos requeridos y se debe asegurar que la transcripción esté lista para ese momento.

**CAMPOS QUE SECRETARÍA TIENE QUE LLENAR**: opcion, notificacion, reunion_terminada.
  
| num | nombre | id_tema | tema | opcion | notificacion | reunion_terminada |
|-----|--------|---------|------|--------|--------------|-------------------|
| 1 | Asistente 1 | mfxkguhme | Tema a tratar 1 | A favor | | |
| 2 | Asistente 2 | mfxkguhme | Tema a tratar 1 | A favor | | |
| 3 | Asistente 1 | mfxkguhm6 | Tema a tratar 2 | A favor | | |
| 4 | Asistente 2 | mfxkguhm6 | Tema a tratar 2 | En contra | | |

  

### Proceso Adicional 1: Agregar nuevos temas

Si durante la reunión surge la necesidad de tratar nuevos temas, es necesario colocarlos al final del documento, siempre llenando las columnas **"nombre"**, **"tema"**, **"tabla"**. La columna **"id_tema"** no debe ser llenada.

  
| num | nombre | id_tema | tema | opción | notificacion | reunion_terminada |
|-----|--------|---------|------|--------|--------------|-------------------|
| 1 | Asistente 1 | mfxkguhme | Tema a tratar 1 | A favor | | |
| 2 | Asistente 2 | mfxkguhme | Tema a tratar 1 | A favor | | |
| 3 | Asistente 1 | mfxkguhm6 | Tema a tratar 2 | A favor | | |
| 4 | Asistente 2 | mfxkguhm6 | Tema a tratar 2 | En contra | | |
| 5 | Asistente 1 | | TEMA NUEVO | | | |
| 6 | Asistente 2 | | TEMA NUEVO | | | |
...
| 1 | Asistente 1 | mfxkguhme | Tema a tratar 1 | A favor | | |
| 2 | Asistente 2 | mfxkguhme | Tema a tratar 1 | A favor | | |
| 3 | Asistente 1 | | TEMA NUEVO | | | |
| 4 | Asistente 2 | | TEMA NUEVO | | | |
...
El robot obtendrá distintos artículos referente a cada uno de los temas tratados en la reunión, y remitirá una url de una aplicación web a secretaría para que se analicen, se deberá abrir el link, utilizar el sistema para ordenar o filtrar los artículos y confirmar.
La mecánica de selección es dejar los artículos en la columna de "Artículos Elegidos". También se tiene un botón para buscar leyes, las cuales pueden ser incluidas.
Se tendrá botones de navegación de derecha a izquierda, los cuales permiten desplazarse por las diferentes resoluciones vistas en la reunión, cuando se llegue a la última resolucion, se podrá confirmar los cambios, presionando el botón de navegación hacia la derecha. 

## **Paso 3:** Generación de Resoluciones:

Posteriormente se generan los distindos documentos de resoluciones los cuales son remitidos primeramente a Rectorado para su firmado y posteriormente a Secretaría.
...
El asistente tiene que descargar todos los archivos, firmarlos y reenviarlos al mismo correo, el robot lo recibe, y lo reenvia consecutivamente a todos los asistentes de la reunión, y una vez firmado por todos, lo remite a secretaría.
En caso de que haya alguna equivocación por parte del asistente, el bot le enviará un correo con el archivo/s faltantes por firmar.

### CASOS ESPECIALES:

Para validar que un documento haya sido firmado el bot analiza el nombre del archivo, por lo cual el asistente no deberá modificar el nombre del mismo, salvo por la etiqueta **<-signed>** la cual se genera en el archivo luego de ser firmado.
...
- CASO 3: Cuando el asistente no envía ningún archivo.
...
ACCIÓN 2: Si el asistente no responde en 2 avisos, se notificará a secretaría.
...
Buen día, La persona [Nombre Asistente] con el correo [correo@isty.edu.ec] ha superado el máximo de notificaciones, para continuar, favor notificarle personalmente que se reenvíe la convocatoria sesión firmada.
...
ids_wrong_emails TEXT[],

remaining_time TEXT,

lives_remaining INTEGER,

filenames_waiting_to_send TEXT[]
...
```
SCRIPT PARA TRAER LISTA DE DOCUMENTOS
````
CREATE OR REPLACE FUNCTION get_unique_document_names()
RETURNS TABLE(documents text) -- Define que la función devolverá una tabla con una columna de texto
LANGUAGE sql
AS $$
  SELECT DISTINCT
